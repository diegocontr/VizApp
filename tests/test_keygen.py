...
def test_hierarchical_basic_preserved_types(sample_plot_data_preserved_types):
    ...
    expected = {"Alpha": {"XGBoost": {}, "LightGBM": {}}, "Beta": {"XGBoost": {}}, "Gamma": {"NN": {}}}
    assert (
        get_hierarchical_parameter_options(sample_plot_data_preserved_types, key_order, preserved_types_in_keys=True)
        == expected
    )
...
def test_hierarchical_deeper_preserved_types(sample_plot_data_preserved_types):
    ...
    expected = {
        "Alpha": {"XGBoost": {0.05: {}, 0.1: {}}, "LightGBM": {0.1: {}}},
        "Beta": {"XGBoost": {0.1: {}}},
        "Gamma": {"NN": {0.01: {}}},
    }
    result = get_hierarchical_parameter_options(
        sample_plot_data_preserved_types, key_order, preserved_types_in_keys=True
    )
...
def test_hierarchical_string_types(sample_plot_data_string_types):
    ...
    expected = {"Alpha": {"XGBoost": {"1": {}, "2": {}}}, "Beta": {"NN": {"1": {}}}}
    assert (
        get_hierarchical_parameter_options(sample_plot_data_string_types, key_order, preserved_types_in_keys=False)
        == expected
    )
...
def test_distinct_basic_preserved_types(sample_plot_data_preserved_types):
    ...
    result = get_all_distinct_parameter_values(
        sample_plot_data_preserved_types, params_of_interest, preserved_types_in_keys=True
    )
    assert result == expected
...
def test_distinct_string_types(sample_plot_data_string_types):
    ...
    assert (
        get_all_distinct_parameter_values(
            sample_plot_data_string_types, params_of_interest, preserved_types_in_keys=False
        )
        == expected
    )
...
def test_distinct_param_not_found(sample_plot_data_preserved_types):
    ...
    expected = {"dataset": ["Alpha", "Beta", "Gamma"], "non_existent_param": []}
    assert (
        get_all_distinct_parameter_values(
            sample_plot_data_preserved_types, params_of_interest, preserved_types_in_keys=True
        )
...
def test_distinct_malformed_key_skipped(sample_plot_data_preserved_types):
    ...
    assert (
        get_all_distinct_parameter_values(data_with_malformed, params_of_interest, preserved_types_in_keys=True)
        == expected
    )
